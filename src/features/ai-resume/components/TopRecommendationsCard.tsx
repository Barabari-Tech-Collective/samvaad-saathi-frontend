            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] p-2 flex flex-col">
                {recs.map((rec, i) => (
                    <div key={i} className={`flex items-center gap-4 p-4 ${i !== recs.length - 1 ? 'border-b border-slate-50' : ''} hover:bg-slate-50/50 transition-colors cursor-pointer rounded-2xl group`}>
                        <div className="w-6 h-6 shrink-0 rounded-full bg-blue-50 text-[#1e58f1] flex items-center justify-center text-xs font-bold">
                            {i + 1}
                        </div>
                        <p className="flex-1 text-[13px] text-slate-700 font-medium leading-snug">{rec}</p>
                        <ArrowRightIcon className="size-4 text-slate-300 group-hover:text-blue-500 transition-colors" />
                    </div>
                ))}
            </div>

            <div className="flex flex-wrap gap-2 pt-2">
                <span className="px-4 py-2 bg-blue-50 text-[#1e58f1] rounded-full text-xs font-bold border border-blue-100">Frontend Developer</span>
