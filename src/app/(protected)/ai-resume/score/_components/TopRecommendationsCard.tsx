      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] p-2 flex flex-col">
        {recs.length > 0 ? (
          recs.map((rec, i) => (
            <div
              key={i}
              className={`flex items-center gap-4 p-4 ${i !== recs.length - 1 ? "border-b border-slate-50" : ""} hover:bg-slate-50/50 transition-colors cursor-pointer rounded-2xl group`}
            >
