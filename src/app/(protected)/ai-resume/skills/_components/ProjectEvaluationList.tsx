      ...
      <div className="flex flex-col gap-4">
        {projects.length > 0 ? (
          projects.map((proj, i) => (
            <div
              key={i}
              className="p-5 rounded-3xl bg-white border border-slate-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]"
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span
                  className={`px-3 py-1 rounded-full text-[11px] uppercase tracking-wider font-bold ${getStatusColor(proj.rating)}`}
                >
                  {proj.rating}
                </span>
              </div>
              <p className="text-[13px] leading-relaxed text-slate-500 font-medium mb-2">
                {proj.feedback}
              </p>
              {proj.projectUrl &&
                proj.projectUrl.trim() !== "" &&
                proj.projectUrl !== "EXTRACTED_URL_OR_EMPTY_STRING" &&
                !proj.projectUrl.includes("github.com/user/project") && (
                  <a
